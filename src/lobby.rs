...
use crate::packets::*;
use crate::game::{Game, Player, Card};
...
impl Lobby {
    ...
    pub fn broadcast(&mut self, packet: &Packet, data: &str) {
        self.rooms.get(&packet.room_id).unwrap().connections.iter().for_each(|client| self.send_message(data, client))
    }

    pub fn player_exists(&mut self, room_id: &Uuid, id: &Uuid) -> bool{
        self.rooms.get(room_id).unwrap().game.players.contains_key(id)
    }

    pub fn get_player(&mut self, room_id: &Uuid, id: &Uuid) -> &Player{
        self.rooms.get(room_id).unwrap().game.players.get(id).unwrap()
    }

    pub fn get_spectator(&mut self, room_id: &Uuid, id: &Uuid) -> &Player{
        self.rooms.get(room_id).unwrap().game.spectators.get(id).unwrap()
    }
} 
...
impl Handler<Connect> for Lobby {
    ...
    fn handle(&mut self, packet: Connect, _: &mut Context<Self>) -> Self::Result {
        self.rooms.entry(packet.lobby_id).or_insert_with(Room::new).connections.insert(packet.self_id);
        ...
        println!("[{}] User is connecting... ({})", packet.lobby_id, packet.self_id);
    }
}
        ...
        if packet.json.get("type").is_some() {
            ...
            match &r#type as &str {
                ...
                "\"REGISTER\"" => {
                    ...
                    match p {
                        Ok(data) => {
                            let host: bool = self.rooms.get(&packet.room_id).unwrap().game.players.len() == 0;
                            let started: bool = self.rooms.get(&packet.room_id).unwrap().game.active;

                            if self.rooms.get(&packet.room_id).unwrap().game.players.contains_key(&packet.id) {
                                self.emit(&packet, &HTMLError::to_json(HTMLError::new(400, "Instance already exists for this websocket.")) );
                                return;
                            }

                            if !started {
                                
                                let p: Player = Player::new(packet.id, self.sessions.get(&packet.id).unwrap(), host, &data.username);
                                
                                self.rooms.get_mut(&packet.room_id).unwrap().game.join(packet.id, p);
                                self.broadcast(&packet, &format!("{} has joined the game", &data.username) );
                                return;
                            }
                            else {
                                self.rooms.get_mut(&packet.room_id).unwrap().game.spectate(packet.id, Player::new(packet.id, self.sessions.get(&packet.id).unwrap(), host, &data.username));
                                self.broadcast(&packet, &format!("{} has joined as spectator.", &data.username) );
                                return;
                            }
                ...
                "\"MESSAGE\"" => {
                    ...
                    if !self.player_exists(&packet.room_id, &packet.id) {
                        self.emit(&packet, &HTMLError::to_json(HTMLError::new(401, "Only registered players can perform actions.")) );
                        return;
                    }
                    ...
                    match p {
                        ...
                        Err(e) => {
                            self.emit(&packet, &HTMLError::to_json(HTMLError::new(400, &e.to_string())) );
                        }
                    }
                ...
                "\"START-GAME\"" => {
                    ...
                    if !host {
                        self.emit(&packet, &HTMLError::to_json(HTMLError::new(401, "Only host can start the game.")) );
                        return;
                    }
